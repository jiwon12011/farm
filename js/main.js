...
function doAction() {
  ...
  switch (a.kind) {
    ...
    case 'board':
      ...
      break;
    case 'restaurant':
      if (S.village.restaurant.status !== 'done') UI.toast("문이 판자로 막혀 있어요… 빛바랜 간판엔 '반디네 부엌'. 할머니가 하던 식당이에요");
    ...
    case 'shrine':
      if (!S.flags.ch4) {
        ...
        UI.showMemory('🌌 4장 — 반딧불이 사라진 밤', [
          '차갑게 식은 돌 제단에 손을 얹자,<br>돌 틈의 덩굴 문양이 희미하게 깨어났다.<br>레시피북과 똑같은 문양이.',
          '『…그 사람이랑 같은 냄새가 나.』<br>돌아보니, 소년 <b>반디</b>가 서 있었다.<br>식당 사진 속, 그 낯익은 얼굴 그대로.',
          '『정령들은 죽은 게 아니야. 배가 고파서 잠든 것뿐.<br>은하는 알고 있었어. 그래서 매년 끓여줬어.<br>따뜻한 걸 바치면… 다시 깨어날 거야.』',
          '달빛에 비친 반디의 발끝은,<br>땅에 닿아 있지 않았다.',
        ], '제단에 요리를 바쳐보자');
        ...
        UI.showMemory('🌠 에필로그 — 달빛 스튜', [
          '김이 오르는 스튜를 제단에 올리자,<br>숲 전체가 크게 숨을 들이쉬었다.',
          '수천 개의 반딧불이 일제히 떠올랐다.<br>그 빛의 한가운데 — 빛으로 된 사슴,<br>대정령 <b>루미</b>가 천천히 고개를 숙였다.',
          '『이 맛이다. 은하가 매년, 달이 가장 밝은 밤에 끓여주던.』<br>『약속은 다시 이어졌다.<br>이 마을의 계절이, 다시 돈다.』',
          '반디가 웃었다. 처음 보는, 아이다운 웃음이었다.<br>『은하 말이 맞았네. …정말 왔어.』',
          '멀리 마을에서 웃음소리가 들려온다.<br>축제의 불빛, 되살아난 부엌, 밭 위를 나는 반딧불.',
          '— 할머니. 별은, 여기서 잘 보여요. 🌾<br><b>반딧불 대축제가 시작됐다! (✨+100)</b>',
        ], '농장은 계속된다');
      } else UI.openShrine();
      break;
...
function openStove(tool) {
  if (!S.flags.recipeBook) {
    ...
    save(S);
    UI.showMemory('📖 1장 — 잠긴 레시피북', [
      '부엌 서랍 깊숙한 곳, 손때 묻은 책 한 권.<br>표지에 작은 무 하나가 수놓아져 있다.',
      '펼치자 대부분의 페이지가 은은히 빛나는 <b>덩굴 문양</b>으로 잠겨 있었다.<br>글씨가 보이는 건 첫 장뿐.',
      '『요리엔 마음이 담긴단다.<br>마음이 닿으면, 책이 답할 거야. — 은하』',
  ...
  for (const id of REPAIR_ORDER) {
    ...
    if (id === 'board' && !S.flags.ch2) {
      ...
      UI.showMemory('🌙 2장 — 헛간의 발자국', [
        '새 게시판 앞에 주민들이 모여 웅성거린다.<br>마을에 참 오랜만의 활기.',
        '그런데 그날 밤 — 헛간 옆 흙 위에서 <b>맨발의 작은 발자국</b>을 보았다.<br>아이 같기도 하고, 아닌 것 같기도 한.',
        '발자국은 숲 쪽으로 이어지다, 문득 허공에서 끊겼다.<br>그 자리엔 희미한 금빛 가루만 남아 있었다.',
        '문득 레시피북이 떠올랐다.<br>『밤의 작은 친구들도 코가 밝거든.』',
      ], '…누구지?');
    } else if (id === 'restaurant' && !S.flags.ch3) {
      S.flags.ch3 = true; save(S);
      UI.showMemory("🍲 3장 — 되살아나는 부엌", [
        "판자를 뜯어내자, '반디네 부엌'의 낡은 간판이 드러났다.<br>— 반디네. 할머니는 왜 이 이름을 지었을까.",
        '『은하 씨 옥수수스프, 그게 그렇게 맛있었는데.』<br>『손주가 왔다며? 그 맛이 날까?』',
        '옛 단골들이 하나둘 문을 두드린다.<br>이제 요리를 진열해 두면, 자리를 비워도 팔릴 것이다.',
        '주방 벽엔 빛바랜 사진 한 장 —<br>앞치마를 두른 할머니 곁에, 낯익은 얼굴의 소년이 서 있다.',
      ], '개업이다!');
    } else {
...
function checkChapter5() {
  ...
  UI.showMemory('🦌 5장 — 할머니의 약속', [
    '온실 유리가 달빛을 머금은 밤,<br>레시피북의 마지막 문양이 스르르 풀렸다.',
    '그리고 숲에서 — 빛으로 된 사슴이 걸어 나왔다.<br>대정령 <b>루미</b>. 반디가 그 곁에 조용히 섰다.',
    '『<b>은하</b>. 네 할머니의 이름이다.<br>매년 가장 밝은 보름밤, 나에게 스튜를 끓여주었지.<br>그 한 그릇이 — 이 마을의 계절을 지키는 약속이었다.』',
    '『은하가 떠나고 약속이 끊겼다.<br>정령들은 굶주려 잠들고, 마을은 빛을 잃었지.<br>…이제 네가 이을 차례다.』',
    '마지막 페이지의 레시피 — <b>달빛 스튜</b>.<br>재료: 별빛벼 · 반딧꽃 · 트러플 · 우유',
    '별빛벼 씨앗은 봄이네 상점에,<br>반딧꽃 씨앗과 <b>별양</b>은 정령의 제단에 있다.',
  ], '약속을 이어가자');
}
